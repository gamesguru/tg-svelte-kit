...
do_check() {
    run_step "Type Check (prepublishOnly)" "cd packages/kit && pnpm prepublishOnly"
    run_step "Type Check (check)" "pnpm run check"
}
...
do_e2e() {
    ensure_playwright
    run_step "E2E Tests (dev)" "pnpm test:cross-platform:dev"
    run_step "E2E Tests (build)" "pnpm test:cross-platform:build"
}
...
do_cross() {
    ensure_playwright
    run_step "Cross-Platform Tests (dev)" "pnpm test:cross-platform:dev"
    run_step "Cross-Platform Tests (build)" "pnpm test:cross-platform:build"
}

do_ssrr() {
    ensure_playwright
    run_step "Server-Side Route Resolution (dev)" "pnpm test:server-side-route-resolution:dev"
    run_step "Server-Side Route Resolution (build)" "pnpm test:server-side-route-resolution:build"
}

do_async() {
    ensure_playwright
    run_step "Svelte Async Tests (dev)" "pnpm test:svelte-async:dev"
    run_step "Svelte Async Tests (build)" "pnpm test:svelte-async:build"
}
...
do_legacy() {
    if [[ -d "website-template-svkit-v2-legacy" ]]; then
        run_step "Build Kit" "cd packages/kit && pnpm build && pnpm check"
        run_step "Legacy Template Install" "cd website-template-svkit-v2-legacy && pnpm install"
        run_step "Legacy Template Tests" "cd website-template-svkit-v2-legacy && pnpm test"
    else
...
do_quick() {
    ...
    run_step "Unit Tests (fast)" "cd packages/kit && pnpm test:unit:dev"
}
